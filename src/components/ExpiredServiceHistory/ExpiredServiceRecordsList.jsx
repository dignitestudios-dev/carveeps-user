...
import PrevServiceCard from "./PrevServiceCard";
import DateModal from "../ServiceHistory/DateModal";
import axios from "axios";
import Cookies from "js-cookie";
import { GlobalContext } from "../../context/GlobalContext";
import { NoData } from "../../assets/export";

const ExpiredServiceRecordsList = () => {
    ...
    <>
      <div className="w-full bg-white rounded-[18px] p-6 flex flex-col items-start gap-6">
        <div className="w-full flex items-start lg:items-center justify-between flex-col lg:flex-row gap-6">
          <h1 className="text-2xl font-bold">{data?.subscriptionPlan?.name}</h1>
          {dataLoading ? (
            <div className="w-full flex items-start lg:items-center justify-between flex-col lg:flex-row gap-6">
        ...
        <div className="w-full grid grid-cols-1 gap-2 md:grid-cols-2 lg:hidden">
          {services?.map((service, index) => {
            return <PrevServiceCard service={service} key={index} />;
          })}
        </div>
      </div>
    </>
...
export default ExpiredServiceRecordsList;
